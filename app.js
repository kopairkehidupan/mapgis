...
// Export buttons
// ===== EXPORT ALL FILES - GeoJSON =====
el('#exportGeojson').onclick = function(){ 
    var fileIds = Object.keys(uploadedFiles);
    ...
    var confirmMsg = 'Export ' + fileIds.length + ' file ke GeoJSON?';
    ...
    fileIds.forEach(function(id, index) {
        ...
        // Tambahkan metadata ke GeoJSON
        gj.metadata = {
            exportedFrom: 'Mini MapGIS',
            ...
            labelSettings: meta.labelSettings,
        ...
        // Generate nama file
        var filename = meta.name.replace(/\.(gpx|geojson|kml)$/i, '') + '.geojson';
        
        // Delay download agar browser tidak block
        setTimeout(function() {
            saveAs(blob, filename);
            exported++;
            
            if (exported === fileIds.length) {
                setTimeout(function() {
                    alert('Berhasil export ' + exported + ' file GeoJSON!');
                }, 500);
            }
        }, index * 300); // Delay 300ms per file
    });
};

// ===== EXPORT ALL FILES - GPX =====
el('#exportGpx').onclick = function(){ 
    ...
    var confirmMsg = 'Export ' + fileIds.length + ' file ke GPX?';
    if (!confirm(confirmMsg)) return;
    ...
    fileIds.forEach(function(id, index) {
        var meta = uploadedFiles[id];
        var gj = meta.group.toGeoJSON();
        var gpx = geojsonToGpx(gj, meta.name);
        
        var blob = new Blob([gpx], {type:'application/gpx+xml'});
        
        // Generate nama file
        var filename = meta.name.replace(/\.(gpx|geojson|kml)$/i, '') + '.gpx';
        ...
        setTimeout(function() {
            ...
            if (exported === fileIds.length) {
                setTimeout(function() {
                    alert('Berhasil export ' + exported + ' file GPX!');
                }, 500);
            }
        }, index * 300);
    });
...
el('#exportKml').onclick = function(){ 
    ...
    var confirmMsg = 'Export ' + fileIds.length + ' file ke KML?';
    if (!confirm(confirmMsg)) return;
    ...
    fileIds.forEach(function(id, index) {
        ...
        // Generate nama file
        var filename = meta.name.replace(/\.(gpx|geojson|kml)$/i, '') + '.kml';
        ...
        setTimeout(function() {
            ...
            if (exported === fileIds.length) {
                setTimeout(function() {
                    alert('Berhasil export ' + exported + ' file KML!');
                }, 500);
            }
        }, index * 300);
    });
...
// ===== DELETE LAYER (tetap untuk single layer) =====
el('#deleteLayer').onclick = function(){ 
    if(!lastSelectedId) return deleteFile(lastSelectedId); 
};
